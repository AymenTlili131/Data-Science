...
class bitboard:
    def __init__ (self):
        ...
        self.volume=np.uint64(0)
        self.profit=np.uint64(0)
        self.val=''
    def __int__(self):
    ...
    """ converts int a to 100 bit long string """
    def get_bytes_bitboard(self)->str: 
        x=(bin(self.value))[2:].zfill(100)
        return x
    ...
    def print_bitboard(self,N):    #TO DO :update this to include symbol instead of O
        ...
        ch=bin(self.value)[2:].zfill(N)
        for i in range(0,N,int(N**0.5)+1):   
            row=ch[i:i+int(N**0.5)][::-1]
            self.val+='\n'
